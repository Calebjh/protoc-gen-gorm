...
// ContactPageORM  eventually this will be replaced with a Page that uses the paging info
type ContactPageORM struct {
	Results []*ContactORM `gorm:"association_foreignkey;foreignkey"`
}

// TableName overrides the default tablename generated by GORM
func (ContactPageORM) TableName() string {
	return ""
}

// ConvertContactPageToORM takes a pb object and returns an orm object
func ConvertContactPageToORM(from ContactPage) (ContactPageORM, error) {
	to := ContactPageORM{}
	var err error
	for _, v := range from.Results {
		if v != nil {
			if tempResults, cErr := ConvertContactToORM(*v); cErr == nil {
...
// ConvertContactPageFromORM takes an orm object and returns a pb object
func ConvertContactPageFromORM(from ContactPageORM) (ContactPage, error) {
	to := ContactPage{}
	var err error
	for _, v := range from.Results {
		if v != nil {
			if tempResults, cErr := ConvertContactFromORM(*v); cErr == nil {
				to.Results = append(to.Results, &tempResults)
			} else {
				return to, cErr
			}
